...
export default function ImportDialog({ open, onClose, onImport }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-white w-full sm:max-w-lg sm:rounded-xl rounded-t-2xl shadow-xl p-5 pb-[max(20px,env(safe-area-inset-bottom))]"
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Paste JSON Claude gave you in chat.
            </p>
          </div>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-700 text-2xl leading-none px-2 -mt-1"
            aria-label="Close"
          >
            ×
          </button>
        ...
        <div className="flex gap-2 mt-4">
          <button
            onClick={paste}
            className="flex-1 px-4 py-3 text-sm rounded-lg bg-slate-100 hover:bg-slate-200 font-medium"
          >
            📋 Paste
          </button>
          <button
            onClick={submit}
            disabled={!text.trim()}
            className="flex-1 px-4 py-3 text-sm rounded-lg bg-slate-900 text-white disabled:opacity-50 font-medium"
          >
            Add to board
          </button>
        </div>
